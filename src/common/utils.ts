...
export function getNFTGuardsP2TR(guardType: GuardType = GuardType.Transfer): {
  p2tr: string;
  tapScript: string;
  cblock: string;
  contract: SmartContract;
  ...
  const transferGuard = new NftTransferGuard();
  const tapleafKeyBurnGuard = Tap.encodeScript(
    burnGuard.lockingScript.toBuffer(),
  );
  const tapleafKeyTransferGuard = Tap.encodeScript(
...
export function getNFTContract(minterP2TR: string, guardsP2TR: string) {
  return new CAT721(minterP2TR, toByteString(guardsP2TR));
}
...
export function getNFTContractP2TR(minterP2TR: string) {
  const { p2tr: guardsP2TR } = getNFTGuardsP2TR();
  return contract2P2TR(getNFTContract(minterP2TR, guardsP2TR));
}
...
export function getClosedNFTMinterContract(
  issuerAddress: string,
  genesisId: ByteString,
  max: int32,
) {
...
export function getNFTOpenMinterContract(
  ...
  return new NftOpenMinter(genesisId, max, premine, premineAddr);
}
...
export function getNftOpenMinterContractP2TR(
  genesisId: ByteString,
  max: int32,
  premine: int32,
  premineAddr: ByteString,
  ...
  return contract2P2TR(
    getNFTOpenMinterContract(genesisId, max, premine, premineAddr),
  );
}
...
export function getNftClosedMinterContractP2TR(
  issuerAddress: string,
  genesisId: ByteString,
  max: int32,
) {
